#!/usr/bin/env python3
"""
Improved financial data extraction with XBRL context filtering based on pre.xml analysis.
"""
...
def analyze_pre_xml_structure(pre_xml_path):
    ...
    try:
        ...
        for role_ref in root.findall('.//{http://www.xbrl.org/2003/linkbase}roleRef'):
            role_uri = role_ref.get('roleURI', '')
            if 'Statement' in role_uri or 'Segment' in role_uri:
                statement_roles[role_uri] = {
                    'type': 'statement' if 'Statement' in role_uri else 'segment',
                    'elements': []
                }
        ...
        for presentation_link in root.findall('.//{http://www.xbrl.org/2003/linkbase}presentationLink'):
            ...
            for presentation_arc in presentation_link.findall('.//{http://www.xbrl.org/2003/linkbase}presentationArc'):
...
def extract_financials_with_context_filtering(conn, pre_xml_analysis=None):
    """Extract financial data with context filtering based on pre.xml analysis."""
    cursor = conn.cursor()
    ...
    for row in results:
        ...
        if context_id:
            ...
            # Segment data often has longer, more complex context IDs
            if len(context_id) < 20 and 'Segment' not in context_id:
...
def filter_main_statement_data(financial_data):
    """Filter to keep only main statement data, excluding segment breakdowns."""
    main_data = []
    ...
    for key, data_list in grouped_data.items():
        # Prefer main statement context
        main_statement_data = [d for d in data_list if d['context_type'] == 'main_statement']
        
        if main_statement_data:
            # If we have main statement data, use the first one (should be the total)
            main_data.append(main_statement_data[0])
        else:
            # Fallback: use the value with the simplest context ID
            simplest_context = min(data_list, key=lambda x: len(x['context_id']) if x['context_id'] else 999)
            main_data.append(simplest_context)
    
    return main_data

def generate_improved_report(financial_data, output_file=None):
    """Generate a comprehensive financial report with improved data quality."""
    # Filter to main statement data
    main_data = filter_main_statement_data(financial_data)
    ...
    print("=" * 100)
    print("IMPROVED FINANCIAL ANALYSIS - MAIN STATEMENT DATA ONLY")
    print("=" * 100)
    ...
    if output_file:
        ...
        print(f"\nImproved data saved to: {output_file}")
...
def main():
    ...
    try:
        print("Extracting financial data with context filtering...")
        financial_data = extract_financials_with_context_filtering(conn)
        ...
        output_file = "improved_financial_analysis.csv"
        validated_df = generate_improved_report(financial_data, output_file)
